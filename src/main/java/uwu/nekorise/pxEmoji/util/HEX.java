...
public class HEX {
    ...
    private static String findAndReplaceRegex(String regex, String input) {
        ...
        while (matchIterator.hasNext() && colorIterator.hasNext())
            input = input.replaceFirst(matchIterator.next(), ((ChatColor)colorIterator.next()).toString());
        return input;
    }
    ...
    public static String rainbowText(String message) {
        ...
        for (int messageChar = 0; messageChar < (message.toCharArray()).length; messageChar++) {
            ...
            String hexColor = "!#" + ((red.length() <= 2) ? (StringUtils.repeat("0", 2 - red.length()) + red) : red) + ((green.length() <= 2) ? (StringUtils.repeat("0", 2 - green.length()) + green) : green) + ((blue.length() <= 2) ? (StringUtils.repeat("0", 2 - blue.length()) + blue) : blue);
    ...
    public static String gradientText(String message, Color startColor, Color endColor) {
        StringBuilder finalizedMessage = new StringBuilder();
        for (int messageChar = 0; messageChar < (message.toCharArray()).length; messageChar++) {
            float ratio = messageChar / (message.toCharArray()).length;
            int red = (int)(endColor.getRed() * ratio + startColor.getRed() * (1.0F - ratio));
            int green = (int)(endColor.getGreen() * ratio + startColor.getGreen() * (1.0F - ratio));
            int blue = (int)(endColor.getBlue() * ratio + startColor.getBlue() * (1.0F - ratio));
            ...
            String hexColor = "!#" + ((redHex.length() <= 2) ? (StringUtils.repeat("0", 2 - redHex.length()) + redHex) : redHex) + ((greenHex.length() <= 2) ? (StringUtils.repeat("0", 2 - greenHex.length()) + greenHex) : greenHex) + ((blueHex.length() <= 2) ? (StringUtils.repeat("0", 2 - blueHex.length()) + blueHex) : blueHex);
            finalizedMessage.append(hexColor).append(message.toCharArray()[messageChar]);
        }
        return applyColor(finalizedMessage.toString());
    }
